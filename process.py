...
for n in range(1, 10):
    _1_to_9[n] = [list(filter(lambda w: (len(w) == n) and w.isalpha(), master))]

with open(PATH_LISTS / f'1_9_alpha.json', 'w', encoding='utf-8') as f:
    f.write(json.dumps(words_n, ensure_ascii=False))
with open(PATH_JS / f'1_9_alpha.js', 'w', encoding='utf-8') as f:
    f.write(f'const fr_1_9_alpha = {json.dumps(words_n_alpha, ensure_ascii=False)};')
